   ...
   som matcher høyden på dokumentet */
	...
	static settKorrektNavigering() {
		$.get("Brukere/AdminTilgang", function (innlogget) {
			if (!innlogget) {
				$("nav-bar").append(
					`<a slot="innlogging" id="innlogging" href="../loggInn.html">Logg inn</a>`
				);
			} else {
				$("nav-bar").append(
					`<a slot="innlogging" id="innlogging" href="../index.html">Logg ut</a>`
				);
				$("nav-bar").append(`<span slot="skille">|</span>`);
				$("nav-bar").append(
					`<a slot="admin" href="Adminsider/ruter.html">Admin</a>`
				);
	...
	static leggTilBillett() {
		...
		$("#billetter").append(
			`<select id="${id}" class="form-control billettype mb-2"></select>`
		);
		var dropdown = $(`#${id}`);
