	...
	if (validerReisevalgInput()) {
		...
		$.post("Avgang/FinnNesteAvgang", avgangParam, function (respons) {
			...
			ut = `<div id="billettInfo" class="antialised">
                    <h4 id="billettInfoOverskrift"><strong>${
											avgang.Rutenavn
										}, ${avgang.Linjekode}</strong></h4>
                    <div id="billettInfoBody">
                        <h6>
                            Avreise: ${Hjelpemetoder.formaterDatoOgTid(
															avgang.Avreise,
															"dato"
														)} &nbsp;|&nbsp;
                            ${StartStopp}, ${Hjelpemetoder.formaterDatoOgTid(
				avgang.Avreise,
				"tid"
			)}
                            &nbsp;→&nbsp; ${SluttStopp}, ${Hjelpemetoder.formaterDatoOgTid(
				avgang.Ankomst,
				"tid"
			)}
                        </h6>
                        <h6 class="mt-4">
                            Reisetid: ${Hjelpemetoder.finnReisetid(
															avgang.Reisetid
														)}
                        </h6>
                        <h6 class="mt-4">
                            Billetter:${Hjelpemetoder.formaterValgteBillettyper(
															Hjelpemetoder.hentValgteBillettyper()
														)}
                        </h6>
                        <h6 class="mt-4">
                            Pris: ${avgang.Pris} kr
...
function fullforOrdre() {
	...
	// Lager en kundeordre
	const kundeordre = new KundeOrdre(
		epost,
		StartStopp,
		SluttStopp,
		avgang.Linjekode,
		avgang.AvgangId,
		avgangParam.Billettyper
	);
